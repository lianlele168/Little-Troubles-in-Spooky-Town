...
const storageKey = "little-troubles-completed-tasks-v1";
...
export default function TaskTracker() {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      try {
        ...
        if (Array.isArray(saved)) setCompleted(saved.filter((id) => Number.isInteger(id) && id >= 1 && id <= townTasks.length));
      } catch {
  ...
  const visibleTasks = useMemo(() => townTasks.filter((task) => {
    if (filter === "done") return completed.includes(task.id);
    if (filter === "remaining") return !completed.includes(task.id);
    return true;
  }), [completed, filter]);

  const nextTask = townTasks.find((task) => !completed.includes(task.id));
  const percent = Math.round((completed.length / townTasks.length) * 100);
  ...
  async function copyRemaining() {
    const remaining = townTasks.filter((task) => !completed.includes(task.id));
    const text = remaining.length
      ? `Little Troubles in Spooky Town - remaining tasks\n${remaining.map((task) => `${task.id}. ${task.title} - ${task.requirement}`).join("\n")}`
      : "Little Troubles in Spooky Town - all 11 tasks complete!";
    ...
    <div className="tracker-shell">
      <div className="tracker-summary">
        <div>
          ...
          <h2>{completed.length} of 11 complete</h2>
          <p>{nextTask ? <>Next: <Link href={`/${nextTask.slug}/`}>{nextTask.title}</Link></> : "Spooky Town is ready for the ending photo."}</p>
        </div>
      ...
      <div className="tracker-toolbar">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => setCompleted(townTasks.map((task) => task.id))} className="icon-button" aria-label="Mark every task complete" title="Mark all complete"><CheckCheck className="h-4 w-4" /></button>
      ...
      <div className="tracker-list">
        ...
        {!visibleTasks.length ? <p className="tracker-empty">No tasks in this view.</p> : null}
      </div>
      <span className="sr-only" aria-live="polite">{copied ? "Remaining task list copied" : `${completed.length} of 11 tasks complete`}</span>
    </div>
  );
}
